import os
import sys
import torch
import shutil
import argparse
import gymnasium as gym
import torch.multiprocessing as mp

import custom_env

from model import ActorCriticFF
from optimizer import SharedAdam
from process import local_train_ff
from torch.utils.tensorboard import SummaryWriter


def get_args():
    parser = argparse.ArgumentParser(
        """
            Implementation model A3C: 
            IMPLEMENTASI ALGORITMA ASYNCHRONOUS ADVANTAGE ACTOR-CRITIC (A3C)
            UNTUK MENGHASILKAN AGEN CERDAS (STUDI KASUS: PERMAINAN TETRIS)
        """
    )
    parser.add_argument("--lr", type=float, default=7e-4, help="Learning rate")
    parser.add_argument(
        "--gamma", type=float, default=0.99, help="discount factor for rewards"
    )
    parser.add_argument("--beta", type=float, default=0.01, help="entropy coefficient")
    parser.add_argument("--tau", type=float, default=0.95, help="lambda for GAE")
    parser.add_argument(
        "--sync-steps",
        type=int,
        default=20,
        help="jumlah step sebelum mengupdate parameter global",
    )
    parser.add_argument(
        "--save-interval",
        type=int,
        default=500,
        help="jumlah step sebelum menyimpan model",
    )
    parser.add_argument(
        "--max-steps", type=int, default=1e5, help="Maksimal step pelatihan"
    )
    parser.add_argument(
        "--num-agents",
        type=int,
        default=10,
        help="Jumlah agen yang berjalan secara asinkron",
    )
    parser.add_argument(
        "--log-path",
        type=str,
        default="tensorboard/a3c_tetris_ff",
        help="direktori plotting tensorboard",
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default="trained_models",
        help="direktori penyimpanan model hasil training",
    )
    parser.add_argument(
        "--render-mode",
        type=str,
        default="rgb_array",
        help="Mode render dari environment",
    )
    parser.add_argument(
        "--load-model",
        type=bool,
        default=False,
        help="Load weight from previous trained stage",
    )
    args = parser.parse_args()
    return args


def model_logger(model_queue: mp.Queue, opt: tuple):
    writer = SummaryWriter(opt.log_path)

    while True:
        while not model_queue.empty():
            log_data = model_queue.get()
            if log_data is None:
                writer.close()
                return

            episode = log_data["episode"]
            
            writer.add_scalar(f"Global/rewards", log_data['mean_reward'], episode)

            if episode % 100:
                writer.flush()


def train(opt):
    try:
        device = "cuda" if torch.cuda.is_available() else "cpu"
        torch.manual_seed(42)
        if os.path.isdir(opt.log_path):
            shutil.rmtree(opt.log_path)
        os.makedirs(opt.log_path)

        if not os.path.isdir(opt.model_path):
            os.makedirs(opt.model_path)

        mp.set_start_method("spawn")
        env = gym.make("SmartTetris-v0")

        global_model = ActorCriticFF((2, 84, 84), env.action_space.n).to(device)
        global_model.share_memory()

        optimizer = SharedAdam(global_model.parameters(), lr=opt.lr)
        optimizer.share_memory()

        del env

        if opt.load_model:
            file_ = "{}/a3c_tetris_ff.pt".format(opt.model_path)
            if os.path.isfile(file_):
                global_model.load_state_dict(torch.load(file_))

        processes = []
        global_steps = mp.Value("i", 0)
        global_episodes = mp.Value("i", 0)
        res_queue = mp.Queue()

        for index in range(opt.num_agents):
            process = mp.Process(
                target=local_train_ff,
                args=(
                    index,
                    opt,
                    global_model,
                    optimizer,
                    global_episodes,
                    global_steps,
                    res_queue,
                ),
            )
            process.start()
            processes.append(process)

        logger_process = mp.Process(
            target=model_logger,
            args=(res_queue, opt)
        )
        logger_process.start()

        for process in processes:
            process.join()

        logger_process.join()

    except (KeyboardInterrupt, mp.ProcessError):
        print("Multiprocessing dihentikan...")
        raise KeyboardInterrupt


if __name__ == "__main__":
    try:
        opt = get_args()
        train(opt)
    except KeyboardInterrupt:
        print("Program dihentikan...")
        sys.exit()
