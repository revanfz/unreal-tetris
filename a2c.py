...
def preprocessing_state(x: torch.Tensor):
...
def batch_samples(samples: np.ndarray, device: torch.device):
    ...
    )  # [n_envs, seq_len, channels, height, width]
    rewards_batch = torch.FloatTensor(np.array(rewards), device=device).unsqueeze(
        -1
    )  # [n_envs, seq_len, 1]
    ...
    pixel_changes_batch = torch.FloatTensor(
        np.array(pixel_changes), device=device
    )  # [n_envs, seq_len, 20, 20]
...
def train_a2c(
    ...
    try:
        ...
        if log:
            writer = SummaryWriter(f"{log_path}/A2C-unreal")
        ...
        state = preprocessing_state(torch.from_numpy(state.transpose(0, 3, 1, 2)))

        action = F.one_hot(
            torch.LongTensor([0 for _ in range(n_envs)]), model.n_actions
        ).to(device)
        ...
        if log:
            with torch.no_grad():
                writer.add_graph(
                    ...
                    (
                        preprocessing_state(
                            torch.from_numpy(next_state.transpose(0, 3, 1, 2))
                        ),
                        action,
                        reward,
                    ),
        ...
        while not (all(buffer._is_full() for buffer in replay_buffer)):
            ...
            for i in range(n_envs):
                ...
                if done[i]:
                    next_hx[i] = torch.zeros_like(next_hx[i])
                    next_cx[i] = torch.zeros_like(next_cx[i])

            state = next_state
            action = F.one_hot(torch.LongTensor(action), num_classes=model.n_actions).to(
                device
            )
        ...
        prev_reward = torch.zeros(n_envs).to(device)
        hx = torch.zeros(n_envs, hidden_size).to(device)
        cx = torch.zeros(n_envs, hidden_size).to(device)
        ...
        for sample_phase in tqdm(range(n_updates)):
            ...
            for step in range(unroll_steps):
                ...
                actions[step] = F.one_hot(
                    torch.LongTensor(action), num_classes=model.n_actions
                ).to(device)
            ...
            with torch.no_grad():
                _, R, _, _ = model(state, prev_action, prev_reward.unsqueeze(1), (hx, cx))
                R = R.squeeze()
            ...
            pc_loss = model.batch_pc_loss(
                states_batch, rewards_batch, actions_batch, dones_batch, pixel_changes_batch
            )
            ...
            (states_batch, rewards_batch, actions_batch, dones_batch, _) = batch_samples(
                samples, device
            )
            ...
            if log:
                ...
                writer.add_scalar(
                    f"Entropy", entropies.detach().mean().cpu().numpy(), sample_phase
                )
                writer.add_scalar(
...
if __name__ == "__main__":
    ...
    critic_loss, actor_loss, pc_loss, rp_loss, vr_loss, entropies, return_queue = (
        train_a2c(
            n_envs=n_envs, device=device, n_updates=n_updates, unroll_steps=unroll_steps, 
            log=False,
            plot=True
        )
    )
